...
import { Plus, Loader2, ImagePlus, Disc3, Check } from 'lucide-react';
...
export default function CreateAlbumPage() {
    ...
    return (
      <div className="mx-auto max-w-xl px-4 sm:px-6 py-24 text-center animate-fade-in">
        <div className="card p-12">
          ...
          <p className="text-sm text-ink-400 mb-6">You need an artist account to create albums.</p>
          <button onClick={() => navigate({ name: 'register' })} className="btn-primary mx-auto">
            Become an artist
          </button>
        </div>
      </div>
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim()) { push('error', 'Please give your album a title'); return; }
    setLoading(true);
    try {
      const album = await createAlbum({ title, description, coverImageFile, coverImageUrl });
      push('success', `Album "${album.title}" created`);
      navigate({ name: 'album', id: album._id });
    } catch (err) {
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 py-10 animate-fade-in">
      <div className="mb-8">
        <h1 className="flex items-center gap-2.5 text-3xl font-bold tracking-tight">
          <Plus className="h-7 w-7 text-brand-400" /> Create Album
        </h1>
        <p className="mt-1 text-sm text-ink-400">Release a new collection of tracks.</p>
      </div>
      ...
      <form onSubmit={submit} className="card p-6 sm:p-8 space-y-6">
        ...
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-ink-300 uppercase tracking-wide">Album title</label>
          <input
            required
            value={title}
        ...
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-ink-300 uppercase tracking-wide">
            Description <span className="normal-case text-ink-600 font-normal">(optional)</span>
          </label>
          <textarea
            value={description}
            ...
            maxLength={500}
          />
          <p className="mt-1 text-right text-[11px] text-ink-600">{description.length}/500</p>
        </div>
